...
pub fn build(b: *std.build.Builder) void {
    ...
    const exe = b.addExecutable("compile_raylib", "src/main.zig");
    ...
    const raylib_srcdir = "raylib/src";

    const raylib = b.addStaticLibrary("raylib", raylib_srcdir ++ "/raylib.h");
    raylib.setTarget(target);
    raylib.setBuildMode(mode);
    raylib.linkLibC();
    raylib.addIncludePath(raylib_srcdir ++ "/external/glfw/include");
    ...
    switch (raylib.target.toTarget().os.tag) {
        .windows => {
            raylib.addCSourceFiles(&.{raylib_srcdir ++ "/rglfw.c"}, raylib_flags);
    ...
    exe.addIncludePath(raylib_srcdir);
